...
describe("Data Access Layer", () => {
    ...
    test("Retrieving non-existent poll should throw error", () => {
        expect(() => retrievePoll(testPoll.ID)).toThrow();
    })
    ...
    test("Should not be able to cast ballot for non-existent poll", () => {
        expect(() => castBallot(testBallot, testPoll.ID)).toThrow();
    })

    xtest("Should not be able to cast two ballots for one user", () => {
        createPoll(testPoll);
        let result = retrievePoll(testPoll.ID);
        expect(result?.ballots).toHaveLength(0);
